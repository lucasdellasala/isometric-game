...
src/
  ...
  core/                      — Pure game logic (no SDL2)
    ...
    pathfinding.rs           — A* with octile heuristic (8 directions)
...
- **Spritesheet tiles:** Extracted from spritesheets to individual 128×64 PNGs with diamond transparency mask
- **Entity sprite pipeline:** Individual PNGs per direction in `assets/sprites/` (for editing) → `scripts/build_spritesheets.py` combines them into spritesheets in `assets/spritesheets/` → game loads spritesheets with `src_rect` per frame. Run `python scripts/build_spritesheets.py` after editing any sprite PNGs
- **Entity sprites at 256×512:** All entity sprites are 256×512 px. `ENTITY_SCALE = 0.33` scales them down. Per-type multipliers (`SCALE_PLAYER`, `SCALE_NPC`, `SCALE_ENEMY_ORC`) for size differences
- **Idle breathing animation:** 8 frames per direction at ~2.5 FPS. Each entity starts at random frame. Falls back to static idle if animated frames don't exist
- **Entity shadow:** `entity_shadow.png` rendered beneath all entities. Scale/offset configurable via debug menu
...
- **Entity outlines:** Pre-computed at load time from static idle PNGs only (not walk/idle animated). Stored as `Vec<(i32,i32)>` per direction. Drawn with `canvas.fill_rect()` for uniform color
...
assets/
  ...
  sprites/                                  — Individual PNGs (source of truth for editing)
    player/
      entity_shadow.png                     — Shared shadow sprite (256×128)
      idle/entity_player_{DIR}.png          — 8 static idle (256×512)
      idle/entity_player_idle_{DIR}_{F}.png — 64 animated idle (256×512)
      walk/entity_player_walk_{DIR}_{F}.png — 64 walk frames (256×512)
    npc/{variant}/
      entity_npc_{variant}_{DIR}.png        — 8 static idle per variant (256×512)
      idle/..._idle_{DIR}_{F}.png           — 64 animated idle per variant
      walk/..._walk_{DIR}_{F}.png           — 64 walk frames per variant
    enemy/{type}/
      entity_{type}_{DIR}.png               — 8 static idle (256×512)
      idle/..._idle_{DIR}_{F}.png           — 64 animated idle
      walk/..._walk_{DIR}_{F}.png           — 64 walk frames
    decorations/grass_tuft_01..08.png       — Grass tufts (16×24)
  spritesheets/                             — Generated by scripts/build_spritesheets.py
    player_idle.png                         — 2048×512 (8 dirs × 1 frame)
    player_idle_anim.png                    — 2048×4096 (8 dirs × 8 frames)
    player_walk.png                         — 2048×4096 (8 dirs × 8 frames)
    npc_{variant}_idle/idle_anim/walk.png   — Same layout per NPC variant
    enemy_{type}_idle/idle_anim/walk.png    — Same layout per enemy type
  tiles/
...
scripts/
  build_spritesheets.py                     — Combines individual PNGs into spritesheets
...
**Player idle sprites:**
- Size: 256×512 px, RGBA with transparent background
